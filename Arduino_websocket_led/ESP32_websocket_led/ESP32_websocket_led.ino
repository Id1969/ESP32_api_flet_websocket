/*
==========================================================================================
 PROYECTO: Control remoto de RELÉ con ESP32-C6 vía WebSocket (WS / WSS) +
Timestamp ISO 8601
==========================================================================================

 OBJETIVO
 --------
 - Controlar un relé (GPIO4) desde un servidor WebSocket (FastAPI).
 - Soportar múltiples ESP32 + múltiples frontends.
 - Protocolo JSON robusto, con timestamps entendibles.

 ARQUITECTURA
 -----------
    FRONTEND(S)  <── WS/WSS ──>  FASTAPI SERVER  <── WS/WSS ──>  ESP32(S)

 PROTOCOLO JSON (coincide con tu main.py)
 ---------------------------------------
 1) Register (ESP32 -> servidor) [SIEMPRE primer mensaje tras conectar]
    {
      "type":"register", "role":"esp32",
      "id":"esp32_01", "mac":"..", "ip":"..",
      "ts":"2025-12-21T19:35:42Z"
    }

 2) Command (frontend -> servidor -> ESP32)
    { "type":"command", "to":"esp32_01", "device":"relay", "id":0, "action":"on"
}

 3) get_state (frontend -> servidor -> ESP32)
    { "type":"get_state", "to":"esp32_01" }

 4) state (ESP32 -> servidor -> frontends)
    { "type":"state", "from":"esp32_01", "device":"relay", "id":0, "state":"on",
"ts":"..." }

 5) Keep-alive
    ESP32 -> servidor: { "type":"ping", "from":"esp32_01", "ts":"..." }
    servidor -> ESP32: { "type":"pong" }

 FIXES IMPORTANTES
 -----------------
 1) WebSocketsClient entrega WStype_TEXT con payload NO garantizado con '\0'.
    → Copiamos payload respetando length (evita basura y "mensajes raros").
 2) ArduinoJson: para máxima robustez, forzamos el root como JsonObject y usamos
    containsKey("type"). Si falla, listamos claves para depuración.
